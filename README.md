...
The `mw19pools` command maps **all 117 Replay pool IDs** and dumps allocated process assets, with optional XPak retrieval for streamed payloads. The `mw19replay` fastfile handler loads **all 111 enabled Replay asset types** from final XFile version `0xff7`, using the matching local Replay executable. It uses the same **19 conventional payload exporters** and structured exporters for the other **92 enabled types**. Images export as DDS, model surfaces and stream keys retain their binary buffers, and sound banks export as SAB. The process command also offers `--geometry` for a base-surface GLB view and `--audio` for supported FLAC samples.

...
The `mw19pools` command maps **all 117 Replay pool IDs** and dumps allocated process assets, with optional XPak retrieval for streamed payloads. The `mw19replay` fastfile handler loads **all 111 enabled Replay asset types** from final XFile version `0xff7`, using the matching local Replay executable. It uses the same **19 conventional payload exporters** and structured exporters for the other **92 enabled types**. Images export as DDS, model surfaces and stream keys retain their binary buffers, and sound banks export as SAB. Both the Replay process command and fastfile handler offer `--geometry` for a base-surface GLB view. The process command also offers `--audio` for supported FLAC samples.

The Replay fastfile exporter preserves stream/alias fixups and script-string indices, retains image pixels before GPU upload, and retrieves required external payloads from explicitly supplied XPaks. Large pointer-free geometry arrays retain their complete typed bytes in structured JSON. Bounded tests loaded Shipment's 10,478 assets and successfully exported one sample from each of 51 populated types in memory, without writing game asset files. See [Replay fastfile usage and validation](docs/mw19/fastfile.md) for the executable requirement, output formats, limits, and test coverage.

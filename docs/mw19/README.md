...
The separate [`mw19replay` fastfile handler](fastfile.md) loads all 111 enabled Replay types from supported revisions shipped in the 1.20 install, including older unchanged transient zones and final XFile version `0xff7`, and exports the same conventional or structured formats. It applies matching `.fp`/`.fc` companions, including header-only revision updates. It maps the owner's matching executable locally and redirects its generated serialization routines to bounded streams; the game entrypoint, renderer and runtime asset registration are not started. Its `--test` mode exercises a bounded sample per type entirely in memory and writes only two reports per zone. Remaining asset errors preserve available output and remain explicit in the report.
...
**Replay clears nonstreamed `GfxImage.pixels` after GPU upload**, confirmed by the native instruction at RVA `0x193888D`. The process reader reports `image_uploaded` when those CPU bytes are unavailable. The `mw19replay` fastfile exporter retains the serialized pixels at the upload callback, before its temporary stream is reused, allowing resident DDS export without a running game. GPU readback remains outside these commands. The game-test console image layout is not treated as the PC layout. See [parser validation](parser-validation.md) for the populated DDL, player-condition, voxel-tree and older frontend lighting corrections, selected full extractions, and remaining layout/input boundaries.
